...
import 'package:flutter_aws_s3/widgets/signin_button.dart';
...
class _MyHomePageState extends State<AuthScreen> {
  ...
  Future<void> _signIn() async {
    ...
    // Check if there is a active session for the Current user
    // If so, log the user out first.
    try {
      if (await auth.fetchSession() == 'true') {
        await auth.signOut();
      }
    } catch (error) {
      print(error);
